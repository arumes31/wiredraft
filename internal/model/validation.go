...
func (t Topology) Validate() error {
	...
	if !idPattern.MatchString(strings.TrimSpace(t.OrganizationID)) {
		return errors.New("topology organization id must be a version 4 uuid")
	}
	organization := strings.TrimSpace(t.Organization)
	location := strings.TrimSpace(t.Location)
	if organization == "" || len(organization) > 120 {
		return errors.New("topology organization must contain 1 to 120 characters")
	}
	if len(location) > 120 {
