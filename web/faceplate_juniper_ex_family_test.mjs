...
import {resolveJuniperEXFamilyFaceplate} from "./static/js/faceplate-juniper-ex-family-models.js";
import {buildFaceplateScene} from "./static/js/faceplate-scene.js";
import {hardwarePrimitives} from "./static/js/hardware-components.js";

const cases=[{model:"EX2300 family",sku:"EX2300-48P",count:56,mapped:49,height:44.5,rack:482.6},
  {model:"EX3400 family",sku:"EX3400-48P",count:58,mapped:51,height:43.7,rack:482}];
...
for(const c of cases) {
  test(`${c.sku} resolves its exact base hardware population`,()=>{
    const d=deviceFor(c),p=resolveJuniperEXFamilyFaceplate(d);assert.equal(p.sku,c.sku);assert.equal(d.ports.length,c.count);
    assert.equal(Object.values(p.faces).flatMap(f=>f.ports).length,c.count);assert.equal(d.ports.filter(p=>p.type==="SFP_PLUS_10G").length,4);
    assert.equal(d.ports.filter(p=>p.type==="USB_MINI_CONSOLE").length,1);assert.equal(d.ports.filter(p=>p.type==="Power").length,1);
    assert.equal(p.faces.rear.components.filter(p=>p.role.startsWith("chassis-fan")).length,2);
...
test("Juniper custom artwork uses guarded fans, blank PSU hardware and three C14 blade contacts",()=>{
  const inlet=hardwarePrimitives({kind:"juniper-ex-c14",x:0,y:0,width:35,height:26});assert.equal(inlet.filter(p=>p.fill==="#c5ced1").length,3);
  const blank=hardwarePrimitives({kind:"panel",variant:"juniper-ex-psu-cover",x:0,y:0,width:100,height:50});assert.ok(blank.length>5);
  const fan=hardwarePrimitives({kind:"fan",variant:"juniper-ex-fan-module",x:0,y:0,width:60,height:50});assert.ok(fan.filter(p=>p.kind==="polygon").length>12);
  const portrait=hardwarePrimitives({kind:"juniper-ex-c14-portrait",x:0,y:0,width:27,height:35}),blades=portrait.filter(p=>p.fill==="#c5ced1");assert.equal(blades.length,3);assert.ok(blades.every(p=>p.width>p.height));
  const lamps=hardwarePrimitives({kind:"status",variant:"juniper-ex-status",role:"system-and-mode-leds",x:0,y:0,width:30,height:24});assert.equal(lamps.filter(p=>p.kind==="circle").length,7);
});

test("EX3400 spare cover grip primitives stay inside the unchanged panel for current/legacy460/690 scenes",()=>{
  for(const width of [460,690])for(const legacy of [false,true]) {
    const d=deviceFor(cases[1],legacy),before=structuredClone(d),scene=buildFaceplateScene(d,{x:0,y:0,width,height:100},{face:"rear"}),cover=scene.components.find(c=>c.role==="spare-psu-cover");
    assert.ok(cover);const profile=resolveJuniperEXFamilyFaceplate(d).faces.rear.components.find(c=>c.role==="spare-psu-cover");
    assert.deepEqual([profile.x,profile.y,profile.width,profile.height],[.787,.02,.177,.96],"Physical cover extent remains source-local and unchanged");
    for(const p of hardwarePrimitives(cover)) {
      ...
      for(const [x,y] of points)assert.ok(x>=cover.x-.001&&x<=cover.x+cover.width+.001&&y>=cover.y-.001&&y<=cover.y+cover.height+.001,`EX3400 cover ${p.kind} at${width}/${legacy}: point${x},${y} outside${cover.x},${cover.y},${cover.width},${cover.height}`);
    }
