...
import {resolveJuniperEXNextFaceplate} from "./static/js/faceplate-juniper-ex-next-models.js";
import {buildFaceplateScene} from "./static/js/faceplate-scene.js";
import {hardwarePrimitives} from "./static/js/hardware-components.js";

const cases=[{model:"EX4100 family",sku:"EX4100-48P",count:60,mapped:54,height:43.7,rack:482.6},
  {model:"EX4300 family",sku:"EX4300-48P",count:61,mapped:51,height:43.7,rack:482}];
...
for(const c of cases) {
  test(`${c.sku} resolves its exact base hardware population`,()=>{
    const d=deviceFor(c),p=resolveJuniperEXNextFaceplate(d);assert.equal(p.sku,c.sku);assert.equal(d.ports.length,c.count);
    ...
    assert.equal(d.ports.filter(p=>p.type===(c.count===60?"USB_C_CONSOLE":"USB_MINI_CONSOLE")).length,1);assert.equal(d.ports.filter(p=>p.type==="Power").length,c.count===60?1:2);
    assert.equal(p.faces.rear.components.filter(p=>p.role.startsWith("chassis-fan")).length,2);
    assert.equal(p.faces.rear.components.filter(p=>p.role==="spare-psu-cover").length,c.count===60?1:0);
    assert.equal(p.faces.rear.ports.filter(p=>p.connectorKind==="qsfp").length,c.count===61?4:0);
...
test("Next Juniper artwork has correct C16 key, portrait C14 and source fan guards",()=>{
  const c16=hardwarePrimitives({kind:"juniper-ex-next-c16",x:0,y:0,width:42,height:31});assert.equal(c16.filter(p=>p.fill==="#c5ced1").length,3);assert.ok(c16.some(p=>p.kind==="polygon"&&p.points.length>=10));
  const portrait=hardwarePrimitives({kind:"juniper-ex-next-c14-portrait",x:0,y:0,width:27,height:35});assert.equal(portrait.filter(p=>p.fill==="#c5ced1").length,3);
  const fan=hardwarePrimitives({kind:"fan",variant:"juniper-ex-next-fan4300",x:0,y:0,width:60,height:50});assert.ok(fan.filter(p=>p.kind==="polygon").length>20);
});

test("EX4100/4300 current and unclaimed source primitives stay within their460/690 component bounds",()=>{
