...
const row = () => hardwareCatalog.find(p => p.vendor === "Palo Alto" && p.model === "PA-5200 family");
...
function deviceFor(legacy = false) {
  ...
  return instantiateProfile(profile, "PA-5220 selected", {x:0,y:0});
}
...
test("PA-5200 alias selects the31-socket PA-5220 and its eight-fan dual-AC rear", () => {
  const d=deviceFor(),p=resolveModelFaceplate(d);
  assert.equal(p.fidelity,"model");assert.equal(p.evidence.selectedModel,"PA-5220");
  assert.equal(d.faceplate.unitsU,3);assert.equal(d.ports.length,31);
  assert.equal(d.ports.filter(p=>p.type==="RJ45_MGIG"&&p.speedMbps===10000).length,4);
  assert.equal(d.ports.filter(p=>p.type==="SFP_PLUS_10G").length,18);
  assert.equal(d.ports.filter(p=>p.type==="QSFP_PLUS_40G").length,5);
  assert.equal(d.ports.filter(p=>p.type==="SFP28_25G"||p.type==="QSFP28_100G").length,0);
  assert.equal(p.faces.front.ports.length,31);assert.equal(p.faces.rear.ports.length,0);
  assert.equal(p.faces.rear.components.filter(c=>c.role==="ac-supply").length,2);
  assert.equal(p.faces.rear.components.filter(c=>c.role==="exhaust-fan").length,8);
  assert.equal(p.faces.rear.components.filter(c=>c.role==="drive").length,4);
  assert.equal(p.faces.front.components.filter(c=>c.role==="status-indicator").length,8);
});

test("PA-5220 preserves the actual original 27-endpoint topology without guessing unsupported sockets", () => {
  for(const sparse of [false,true]) {
    ...
    const supported = new Set([1,2,3,4,25,27]);
    ...
    assert.equal(scene.components.filter(c=>c.ancillarySocket).length,31-scene.ports.length);
...
test("PA-5220 keeps its native 3U proportions and clear captions inside saved allocations", () => {
  for(const width of [460,690])for(const legacy of [false,true])for(const units of [2,3,4]) {
    ...
    const native=deviceFor();const nativeScene=buildFaceplateScene(native,{x:10,y:20,width,height:300},{face:"front"});
    ...
    if(width===690)assert.ok(Math.abs(nativeScene.chassis.height/nativeScene.chassis.width-133.3/438.1)<1e-8);
  }
});

test("PA-5220 AC supplies expose the three source-drawn inlet blades without fictional live status", () => {
...
test("PA-5220 custom hardware strokes remain inside source components at460/690 and1x/2x", () => {
  let checked=0;
  for(const width of [460,690])for(const scale of [1,2])for(const face of ["front","rear"]) {
    const scene=buildFaceplateScene(deviceFor(),{x:0,y:0,width,height:300},{face});
    for(const c of scene.components.filter(c=>c.variant?.startsWith("pa5220-"))) {
