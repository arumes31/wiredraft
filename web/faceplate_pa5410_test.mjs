...
/** Find the exact family alias without relying on catalog order. */
const row = () => hardwareCatalog.find(p => p.vendor === "Palo Alto" && p.model === "PA-5400 family");
...
function deviceFor(legacy = false) {
  ...
  return instantiateProfile(profile, "PA-5410 selected", {x:0,y:0});
}
...
test("PA-5400 selects PA-5410's34 sockets with optical management and three dual-rotor trays", () => {
  const d=deviceFor(),p=resolveModelFaceplate(d);
  assert.equal(p.fidelity,"model");assert.equal(p.evidence.selectedModel,"PA-5410");
  assert.equal(d.faceplate.unitsU,2);assert.equal(d.ports.length,34);
  assert.equal(d.ports.filter(p=>p.type==="RJ45_MGIG"&&p.speedMbps===10000).length,8);
  assert.equal(d.ports.filter(p=>p.type==="SFP_PLUS_10G").length,15);
  assert.equal(d.ports.filter(p=>p.type==="SFP28_25G").length,4);
  assert.equal(d.ports.filter(p=>p.type==="QSFP28_100G").length,4);
  assert.equal(d.ports.filter(p=>p.type==="QSFP_PLUS_40G").length,1);
  assert.equal(p.faces.front.ports.find(p=>p.portIndex===32).type,"SFP_PLUS_10G");
  assert.deepEqual(p.faces.front.ports.filter(p=>p.portIndex>=25&&p.portIndex<=28).map(p=>p.physicalLabel),["41","42","43","44"]);
  assert.equal(p.faces.front.ports.length,34);assert.equal(p.faces.rear.ports.length,0);
  assert.equal(p.faces.rear.components.filter(c=>c.role==="ac-supply").length,2);
  assert.equal(p.faces.rear.components.filter(c=>c.role==="fan-assembly").length,3);
  assert.equal(p.faces.front.components.filter(c=>c.role==="drive-cover").length,1);
  assert.equal(p.faces.front.components.filter(c=>c.role==="status-indicator").length,8);
});

test("PA-5410 preserves the actual original 27-endpoint topology without guessing unsupported sockets", () => {
  for(const sparse of [false,true]) {
    ...
    const supported = new Set([1,2,3,4,5,6,7,8,21,22,23,24,27]);
    ...
    assert.equal(scene.components.filter(c=>c.ancillarySocket).length,34-scene.ports.length);
...
test("PA-5410 keeps its native 2U proportions and clear captions inside saved allocations", () => {
  for(const width of [460,690])for(const legacy of [false,true])for(const units of [2,3,4]) {
    const d=deviceFor(legacy);d.faceplate.unitsU=units;
    const native=deviceFor();const nativeScene=buildFaceplateScene(native,{x:10,y:20,width,height:200},{face:"front"});
    for(const face of ["front","rear"]) {
      const s=buildFaceplateScene(d,{x:10,y:20,width,height:units*100},{face});
      assert.ok(s.chassis.height<=nativeScene.chassis.height+1e-8);
      assert.ok(s.chassis.width<=nativeScene.chassis.width+1e-8);
      assert.ok(Math.abs(s.chassis.height/s.chassis.width-nativeScene.chassis.height/nativeScene.chassis.width)<1e-8);
    ...
    if(width===690)assert.ok(Math.abs(nativeScene.chassis.height/nativeScene.chassis.width-87.4/440.4)<1e-8);
  }
});

test("PA-5410 AC supplies expose the three source-drawn inlet blades without fictional live status", () => {
...
test("PA-5410 custom hardware strokes remain inside source components at460/690 and1x/2x", () => {
  let checked=0;
  for(const width of [460,690])for(const scale of [1,2])for(const face of ["front","rear"]) {
    const scene=buildFaceplateScene(deviceFor(),{x:0,y:0,width,height:200},{face});
    for(const c of scene.components.filter(c=>c.variant?.startsWith("pa5410-"))) {
