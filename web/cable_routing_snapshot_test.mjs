...
import { obstacleAwareCableRoute, pointOnRoute, routeSegments } from "./static/js/cabling.js";
...
for (const fixture of fixtures) {
  ...
  assert.equal(routeSegments(route).every((segment) =>
    segment.source.x === segment.target.x || segment.source.y === segment.target.y), true,
  `${fixture.name} must remain strictly orthogonal`);
