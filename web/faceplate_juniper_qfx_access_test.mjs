...
import {resolveJuniperQFXAccessFaceplate} from "./static/js/faceplate-juniper-qfx-access-models.js";
import {buildFaceplateScene} from "./static/js/faceplate-scene.js";
import {hardwarePrimitives} from "./static/js/hardware-components.js";

const cases=[{model:"QFX5100 family",sku:"QFX5100-48S-AFO",count:59,mapped:2,height:43.7,rack:482.6},
  {model:"QFX5110 family",sku:"QFX5110-48S-AFO",count:58,mapped:2,height:43.7,rack:482.6}];

/** Reconstruct the executed snapshot438 two-unit58-port inventory independently from all current catalog groups. */
function deviceFor(c,legacy=false) {
  const current=hardwareCatalog.find(p=>p.vendor==="Juniper"&&p.model===c.model),catalog=legacy?{...current,units:2,inventoryRevision:0,groups:[
    {zone:"uplink",count:48,type:"SFP28_25G",speed:25000,prefix:"SFP28",poe:false},
    {zone:"uplink",count:8,type:"QSFP_DD_400G",speed:400000,prefix:"QSFP-DD",poe:false},
    {zone:"management",count:1,type:"RJ45_1G",speed:1000,prefix:"MGMT",poe:false},
    {zone:"management",count:1,type:"Console",speed:0,prefix:"CONSOLE",poe:false}]}:current;
...
for(const c of cases) {
  test(`${c.sku} resolves its exact base hardware population`,()=>{
    const d=deviceFor(c),p=resolveJuniperQFXAccessFaceplate(d);assert.equal(p.sku,c.sku);assert.equal(d.ports.length,c.count);
    assert.equal(Object.values(p.faces).flatMap(f=>f.ports).length,c.count);
    assert.equal(d.ports.filter(p=>p.type==="SFP_PLUS_10G").length,48);
    assert.equal(d.ports.filter(p=>p.type==="Power").length,2);
    assert.equal(d.ports.filter(p=>p.type==="SFP_1G").length,c.count===59?1:2);
    assert.equal(p.faces.rear.components.filter(p=>p.role.startsWith("chassis-fan")).length,5);
    assert.equal(p.faces.front.ports.filter(p=>p.connectorKind==="qsfp").length,c.count===59?6:4);
    assert.equal(p.faces.front.components.filter(p=>p.role.includes("output")||p.role==="grandmaster-clock-input").length,c.count===59?0:3);
  ...
  test(`${c.sku} preserves full old identities and maps only source-compatible sockets`,()=>{
    for(const legacy of [false,true]) {
      ...
      assert.equal(d.faceplate.unitsU,legacy?2:1,"Original2U allocation is preserved while new selected hardware is1U");
      ...
      if(legacy)assert.deepEqual(scenes.flatMap(s=>s.ports.map(p=>p.port.portIndex)).sort((a,b)=>a-b),[57,58],"Only actual original management and console identities map");
...
test("QFX timing and exact shared PSU hardware stay distinct from network sockets",()=>{
  const inlet=hardwarePrimitives({kind:"juniper-qfx-access-c14-portrait",x:0,y:0,width:28,height:35});
  assert.equal(inlet.filter(p=>p.fill==="#c5ced1").length,3);assert.ok(inlet.some(p=>p.kind==="polygon"&&p.points.length===42));
  const smb=hardwarePrimitives({kind:"coax",variant:"juniper-qfx-access-smb",x:0,y:0,width:10,height:10});
  assert.equal(smb.length,4);assert.ok(smb.every(p=>p.kind==="circle"),"SMB has no BNC bayonet or RJ45 outline");
});

test("QFX5100/5110 current and unclaimed source primitives stay within their460/690 component bounds",()=>{
