...
test("documented connector panel roles distinguish networking, AV switches, servers and APs", () => {
  for (const [model, face] of [
    ...
    ["Dominion Serial family", "rear"], ["PowerEdge R650", "rear"], ["ProLiant ML350", "rear"],
    ...
    assert.equal(profile.faces[face].ports.length, device.ports.length, model);
  }
});
...
test("configurable equipment identifies schematic service areas without invented fan or PSU populations", () => {
  for (const model of ["PowerEdge R650", "ProLiant DL380", "RackStation family", "PowerStore family", "FAS family",
