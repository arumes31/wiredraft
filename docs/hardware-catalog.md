...
The built-in catalog contains 541 offline schematic profiles across network, compute, storage, facilities, console/OOB, wireless, carrier/broadband edge, and passive-cabling families. A profile records the front-facing connector groups, ordered printed labels, evidence fidelity, and normalized faceplate positions used to generate persistent ports.
...
Use **Device family** to narrow the installer before selecting a provider. Dedicated choices cover access points, carrier handoffs, modems/ONTs, LTE/5G routers, switches, firewalls, routers, and server/infrastructure equipment. Then use **Filter models / SKU** to search values such as `FortiAP`, `RUTX50`, `FG-200G`, `FS-124F`, or `FortiSwitch Rugged`. The summary distinguishes verified panels, family-equivalent legacy panels, generic archetypes, edge placement, and modular chassis whose interfaces depend on installed modules.
...
Supported connector types are `RJ45_1G`, `RJ45_MGIG`, `RJ45_10G`, `DSL_RJ11`, `COAX_F`, `SFP_1G`, `SFP_PLUS_10G`, `SFP28_25G`, `SFP56_50G`, `QSFP_PLUS_40G`, `QSFP28_100G`, `QSFP56_200G`, `QSFP_DD_400G`, `CFP_100G`, `CFP2_100G`, `CFP4_100G`, `OSFP_800G`, `FIBER_LC`, `FIBER_SC`, `FIBER_MPO`, `USB_MICRO_CONSOLE`, `USB_C_CONSOLE`, `Stack`, `Console`, and `Power`. Connector-specific faceplate geometry distinguishes optical cages, fiber couplers, coax service inputs, USB console sockets, management ports, and stack/VSF connectors.
...
The vector faceplate renderer is informed by official front-panel imagery and hardware guides while remaining fully offline. All 541 built-in profiles resolve to a sourced chassis family, and their exact connector populations and normalized positions remain profile-specific. See [Faceplate visual research](faceplate-research.md) for coverage, physical observations, and primary sources.

## Wireless, carrier, broadband, and cellular edge

The general Device installer includes four dedicated edge families:

- **Access Points** — generic single/dual-uplink APs plus Cisco Catalyst 9166I, HPE Aruba AP-635, Fortinet FortiAP 231F, and Ubiquiti UniFi U7 Pro.
- **Carrier Handoffs** — generic copper, 1G optical, and 10G optical NNI/UNI demarcations plus an ADTRAN FSP 150-GE104 family profile.
- **Modems & ONTs** — DOCSIS 3.1, VDSL2, GPON, and XGS-PON endpoints plus Ubiquiti UniFi Cable Internet.
- **LTE / 5G Routers** — generic cellular routers plus Fortinet FortiExtender 511F, Teltonika RUTX50, and Cisco Catalyst IR1101.

These devices use ordinary persistent ports and can connect to multiple peer devices when the selected profile exposes multiple interfaces. Ceiling, wall, and DIN devices are placed free-floating by default; rack placement remains available for shelf-mounted documentation. `COAX_F` represents the F-type RF input on a cable modem, so its service side no longer needs to be modeled as a fake Ethernet jack. Family is catalog metadata only: installed devices retain the stable `AccessPoint`, `Modem`, or `Router` topology category.
...
## Patch panels and rear permanent links

The built-in **Generic Patch** profiles are installed through **+ Panel**, not the general Device provider list. The Panel installer includes Cat5e, Cat6, Cat6a, LC, SC, and MPO panels and keeps their catalog-defined connector type, capacity, rack height, and faceplate layout.

Each numbered patch-panel port has independent front and rear occupancy. Use an ordinary canvas cable for the visible front jack. After installing at least two panels, use **Rear Map** to connect a source backport range to a target backport range one-to-one—for example, panel A ports 1–6 to panel B ports 1–6, then panel A ports 7–9 to panel C ports 1–3. A rear mapping does not consume the front jack, change a passive-panel status LED, or participate in a Trunk/LACP/MC-LAG/Failover cable group.

Rear mappings are created as one atomic operation: a mismatched connector type, out-of-range port, or previously occupied rear termination rejects the complete range. The canvas shows permanent rear runs as dashed amber cables; hover or select one to see both backport endpoints and remove only that rear map.
