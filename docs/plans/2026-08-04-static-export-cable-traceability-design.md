...
Pure routing tests cover 9px gutters, 8px preferred faceplate lanes, order-independent horizontal bridge ownership, 4px arc metadata, endpoint clearance, and unchanged Manhattan base geometry. Export tests verify SVG arc commands, bridge layering, endpoint badges, safe text escaping, and outlined colored paths. Canvas contract tests verify arc drawing and export-only endpoint labels. The JavaScript suite, Go suite, Chromium export workflow, Docker build, health check, and live-container smoke test remain release gates.
